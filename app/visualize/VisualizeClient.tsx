...
export default function VisualizeClient() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error?.message ?? "Request failed. Please try again.");
        ...
        <div className="grid gap-8 md:grid-cols-12 items-start animate-in slide-in-from-bottom-5 duration-500">
            ...
            {result.image ? (
              ...
              <div className="rounded-2xl border border-dashed p-10 text-center bg-card/30">
                <p className="text-sm text-muted-foreground">
                  No matching items found for this concept.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="md:col-span-5 space-y-4">
            <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">
              {result.items.length} Matching Items
            </h2>
            {result.items.length > 0 ? (
              ...
              <p className="text-xs text-muted-foreground italic">No visual components found.</p>
            )}
          </div>
        </div>
      )}
