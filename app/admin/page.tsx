...
export default function AdminPage() {
  ...
  async function runScraper() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error?.message ?? "Scraper pipeline failed.");
      setRunResult(json as RunResult);
      await fetchStatus();
    } catch (err) {
      setRunError(err instanceof Error ? err.message : "An unknown error occurred.");
    } finally {
      setRunning(false);
    }
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage platform operations and view analytics.</p>
        </div>
        <button
          onClick={fetchStatus}
      ...
      <section className="space-y-4 rounded-2xl border border-border/40 bg-card/45 p-6 backdrop-blur-md shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">Run data crawlers manually and view execution history.</p>
          </div>
          <button
            onClick={runScraper}
            disabled={running}
